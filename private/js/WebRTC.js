			...
			if (!document.querySelector(`#${peerConnection}`)) {
				...
				peerConnection.addEventListener('track', async (event) => {
					...
					setTimeout(() => {
						...
						document.querySelector(`#${idName}`).srcObject = remoteStream
					}, 0)
				})
				// console.log(peerConnection)
				...
				peerConnectionList.map(async function s(x) {
					if (x.socketID != selfInfo.stocketioID) {
						...
						setTimeout(async () => {
							...
							let offer = await x.peerConnection.createOffer()
						await x.peerConnection.setLocalDescription(offer)
							let offerData =[offer,x.socketID,selfInfo.stocketioID] 
...
socket.on(`receiveOffer`,async function(receiveofferData) { 
	...
	// console.log(offer)
	let index = peerConnectionList.map(object => object.socketID).indexOf(receiveofferData[2]);
	...
	setTimeout(async () => {	//===============google bug=====????
	let answer = await peerConnectionList[index].peerConnection.createAnswer()
	await peerConnectionList[index].peerConnection.setLocalDescription(answer)
	let answerData = [answer, peerConnectionList[index].socketID, selfInfo.stocketioID]
	console.log(answer)
...
socket.on('receiveAnswer',async function (answerData){
	...
	let index = peerConnectionList.map(object => object.socketID).indexOf(answerData[2]);
	setTimeout(async () => {	//===============google bug=====????
	peerConnectionList[index].peerConnection.setRemoteDescription(answerData[0])
	},10)
