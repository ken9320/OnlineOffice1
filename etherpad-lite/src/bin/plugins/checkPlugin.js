...
;(async () => {
	...
	const epRootDir = await fsp.realpath(
		path.join(await fsp.realpath(__dirname), '../../..')
	)
	...
	const updateDeps = async (parsedPackageJson, key, wantDeps) => {
		...
		for (const [pkg, verInfo] of Object.entries(wantDeps)) {
			const { ver, overwrite = true } =
				typeof verInfo === 'string' || verInfo == null
					? { ver: verInfo }
					: verInfo
			if (deps[pkg] === ver || (deps[pkg] == null && ver == null))
				continue
			if (deps[pkg] == null) {
				...
				logger.warn(
					`Dependency mismatch in ${key}: '${pkg}': '${ver}' (current: ${deps[pkg]})`
				)
			}
			if (autoFix) {
	...
	const prepareRepo = () => {
		...
		if (indexStatus !== '')
			throw new Error(
				`uncommitted staged changes to files:\n${indexStatus}`
			)
		let br
		if (autoCommit) {
		...
		if (autoPush) {
			if (!['master', 'main'].includes(br))
				throw new Error('master/main not checked out')
			execSync('git rev-parse --verify @{u}')
			execSync('git pull --ff-only', { stdio: 'inherit' })
			if (execSync('git rev-list @{u}...') !== '')
				throw new Error('repo contains unpushed commits')
		}
	}
	...
	const checkFile = async (srcFn, dstFn, overwrite = true) => {
		...
		try {
			...
			/* treat as if the file doesn't exist */
		}
		try {
	...
	if (autoPush) {
		logger.warn(
			'Auto push is enabled, I hope you know what you are doing...'
		)
	}
	...
	const readMeFileName = files.filter(
		(f) => f === 'README' || f === 'README.md'
	)[0]
	...
	await Promise.all(
		workflows.map(async (fn) => {
			await checkFile(
				`src/bin/plugins/lib/${fn}`,
				`.github/workflows/${fn}`
			)
		})
	)
	...
	if (!files.includes('package.json')) {
		...
		const packageJSON = await fsp.readFile(`${pluginPath}/package.json`, {
			encoding: 'utf8',
			flag: 'r'
		})
		...
		if (
			checkEntries(parsedPackageJSON, {
				funding: {
					type: 'individual',
		...
		if (
			checkEntries(parsedPackageJSON.scripts, {
				lint: 'eslint .',
				'lint:fix': 'eslint --fix .'
			})
		)
	...
	if (!files.includes('package-lock.json')) {
		...
		if (!autoFix) {
			logger.warn(
				'Run npm install in the plugin folder and commit the package-lock.json file.'
			)
		}
	}
	...
	if (!readMeFileName) {
		...
		if (autoFix) {
			...
			logger.info(
				'please edit the README.md file further to include plugin specific details.'
			)
			await fillTemplate(
	...
	if (!files.includes('CONTRIBUTING') && !files.includes('CONTRIBUTING.md')) {
		...
		if (autoFix) {
			logger.info(
				'Autofixing missing CONTRIBUTING.md file, please edit the CONTRIBUTING.md ' +
					'file further to include plugin specific details.'
			)
			await fillTemplate(
				'src/bin/plugins/lib/CONTRIBUTING.md',
				`${pluginPath}/CONTRIBUTING.md`
			)
		}
	}

	if (readMeFileName) {
		let readme = await fsp.readFile(`${pluginPath}/${readMeFileName}`, {
			encoding: 'utf8',
			flag: 'r'
		})
	...
	if (!files.includes('.gitignore')) {
		logger.warn(
			'.gitignore file not found, please create.  .gitignore files are useful to ' +
		...
		let gitignore = await fsp.readFile(`${pluginPath}/.gitignore`, {
			encoding: 'utf8',
			flag: 'r'
		})
	...
	if (files.includes('templates') && !files.includes('locales')) {
		logger.warn(
			'Translations not found, please create.  ' +
	...
	if (files.includes('npm-debug.log')) {
		logger.warn(
			'npm-debug.log found, please remove.  npm-debug.log should never be commited to ' +
				'your repository.'
		)
		if (autoFix) {
	...
	if (files.includes('static')) {
		...
		if (!staticFiles.includes('tests')) {
			logger.warn(
				'Test files not found, please create tests.  https://github.com/ether/etherpad-lite/wiki/Creating-a-plugin#writing-and-running-front-end-tests-for-your-plugin'
			)
		}
	} else {
		logger.warn(
	...
	try {
		const d = await fsp.realpath(
			path.join(pluginPath, 'node_modules/ep_etherpad-lite')
		)
		assert.equal(d, epSrcDir)
	} catch (err) {
		execSync(`${npmInstall} --no-save ep_etherpad-lite@file:${epSrcDir}`, {
			stdio: 'inherit'
		})
	}
	// linting begins
	try {
		...
		logger.info(
			'Manual linting probably required, check with: npm run lint'
		)
	}
	// linting ends.
	...
	if (autoFix) {
		...
		if (!unchanged) {
			...
			execSync(
				'git read-tree HEAD; git add -A && git diff-index -p --cached HEAD && echo ""',
				{
					env: {
						...process.env,
						GIT_INDEX_FILE: '.git/checkPlugin.index'
					},
					stdio: 'inherit'
				}
			)
			...
			if (autoCommit) {
				...
				logger.info(
					'Fixes applied. Check the above git diff then run the following command:'
				)
			...
			if (autoPush) {
				...
				logger.info(
					'Changes committed. To push, run the following command:'
				)
