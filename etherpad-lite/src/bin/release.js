...
const readJson = (filename) =>
	JSON.parse(fs.readFileSync(filename, { encoding: 'utf8', flag: 'r' }))
...
const assertWorkDirClean = (opts = {}) => {
	...
	if (u !== '')
		throw new Error(
			`untracked files in working directory ${opts.cwd}:\n${u}`
		)
	const s = runc('git diff-index --cached --name-status HEAD', opts)
	if (s !== '')
		throw new Error(
			`uncommitted changes in working directory ${opts.cwd}:\n${s}`
		)
}
...
const assertUpstreamOk = (branch, opts = {}) => {
	...
	if (!new RegExp(`^refs/remotes/[^/]+/${branch}`).test(upstream)) {
		throw new Error(
			`${branch} should track origin/${branch}; see git branch --set-upstream-to`
		)
	}
	try {
		...
		throw new Error(
			`${branch} is ahead of origin/${branch}; do you need to push?`
		)
	}
}
...
if (!dirExists('../ether.github.com')) {
	throw new Error(
		'please clone documentation repo: ' +
...
if (!changelog.startsWith(`# ${newVersion}\n`)) {
	throw new Error(
		`No changelog record for ${newVersion}, please create changelog record`
	)
}
...
try {
	...
	run(
		`git rev-parse -q --verify refs/tags/'${newVersion}' >/dev/null || exit 0; ` +
...
try {
	...
	run(`rm -f latest && ln -s 'v${newVersion}' latest`, {
		cwd: '../ether.github.com/doc/'
	})
...
console.log(
	'  git log --graph --date-order --boundary --oneline --decorate develop@{u}..develop'
)
...
console.log(
	'Visit https://github.com/ether/etherpad-lite/releases/new and create a new release ' +
		`with 'master' as the target and the version is ${newVersion}.  Include the windows ` +
		'zip as an asset'
)
console.log(
	'Once the new docs are uploaded then modify the download links (replace ' +
