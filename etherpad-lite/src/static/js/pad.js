...
const handshake = async () => {
	...
	socket.on('reconnect_failed', (error) => {
		// pad.collabClient might be null if the hanshake failed (or it never got that far).
		if (pad.collabClient != null) {
			pad.collabClient.setChannelState(
				'DISCONNECTED',
				'reconnect_timeout'
			)
		} else {
	...
	socket.on('message', (obj) => {
		...
		if (obj.accessStatus) {
			...
			if (window.clientVars.sessionRefreshInterval) {
				const ping = () =>
					$.ajax('../_extendExpressSessionLifetime', {
						method: 'PUT'
					}).catch(() => {})
...
const pad = {
	...
	_afterHandshake() {
		...
		const postAceInit = () => {
			...
			pad.changeViewOption(
				'padFontFamily',
				padcookie.getPref('padFontFamily')
			)
			$('#viewfontmenu')
		...
		$('body').addClass(
			window.clientVars.readonly ? 'readonly' : 'readwrite'
		)
	...
	handleChannelStateChange: (newState, message) => {
		...
		const wasConnecting =
			padconnectionstatus.getStatus().what === 'connecting'
