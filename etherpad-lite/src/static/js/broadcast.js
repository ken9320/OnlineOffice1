...
const loadBroadcastJS = (
	...
	const padContents = {
		...
		apool: new AttribPool().fromJsonable(
			clientVars.collab_client_vars.apool
		),
		...
		splice(start, numRemoved, ...newLines) {
			...
			for (let i = 0; i < newLines.length; i++) {
				newDivs.push(
					this.lineToElement(newLines[i], this.alines[start + i])
				)
			}
	...
	const applyChangeset = (
		...
		if (
			...
			if (lineChanged === undefined) {
				const [op] = Changeset.deserializeOps(
