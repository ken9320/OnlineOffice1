...
;(function ($) {
	...
	$._farbtastic = function (container, options) {
		...
		fb.initWidget = function () {
			...
			fb.square =
				Math.floor((fb.radius - options.wheelWidth / 2) * 0.7) - 1
				...
				if (i > 0) {
					if (browser.msie) {
						...
						var corr =
							...
							n
						color1 = fb.pack(
							fb.HSLToRGB([d1 - 0.15 * corr, 1, 0.5])
						)
						color2 = fb.pack(
							fb.HSLToRGB([d2 + 0.15 * corr, 1, 0.5])
						)
		...
		fb.updateDisplay = function () {
			...
			fb.invert =
				fb.rgb[0] * 0.3 + fb.rgb[1] * 0.59 + fb.rgb[2] * 0.11 <= 0.6
		...
		fb.mousemove = function (event) {
			...
			if (fb.circleDrag) {
				...
				var sat = Math.max(
					0,
					Math.min(1, -(pos.x / fb.square / 2) + 0.5)
				)
				var lum = Math.max(
					0,
					Math.min(1, -(pos.y / fb.square / 2) + 0.5)
				)
