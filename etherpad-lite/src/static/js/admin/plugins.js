...
$(document).ready(() => {
	...
	const displayPluginList = (plugins, container, template) => {
		plugins.forEach((plugin) => {
			...
			for (const attr in plugin) {
				if (attr === 'name') {
					// Hack to rewrite URLS into name
					const link = $('<a>')
						.attr(
							'href',
							`https://npmjs.org/package/${plugin.name}`
						)
	...
	const sortPluginList = (plugins, property, /* ASC?*/ dir) =>
		plugins.sort((a, b) => {
	...
	const updateHandlers = () => {
		...
		$('.do-uninstall')
			...
			.click((e) => {
				...
				installed.list = installed.list.filter(
	...
	socket.on('results:search', (data) => {
		...
		// Update sorting head
		$('.sort').removeClass('up down').addClass('none')
		...
		if (search.results.length > 0) {
			displayPluginList(
				search.results,
				searchWidget.find('.results'),
				searchWidget.find('.template tr')
			)
		} else {
	...
	socket.on('results:installed', (data) => {
		...
		// filter out epl
		installed.list = installed.list.filter(
			(plugin) => plugin.name !== 'ep_etherpad-lite'
		)
	...
	socket.on('finished:install', (data) => {
		if (data.error) {
			if (data.code === 'EPEERINVALID') {
				alert(
					"This plugin requires that you update Etherpad so it can operate in it's true glory"
				)
			}
			alert(
				`An error occurred while installing ${data.plugin} \n${data.error}`
			)
	...
	socket.on('finished:uninstall', (data) => {
		if (data.error) {
			alert(
				`An error occurred while uninstalling the ${data.plugin} \n${data.error}`
			)
		}
