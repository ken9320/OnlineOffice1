	...
	return (self = {
		...
		setBaseAttributedText: (atext, apoolJsonObj) => {
			aceCallbacksProvider.withCallbacks('setBaseText', (callbacks) => {
				...
				if (apoolJsonObj) {
					const wireApool = new AttributePool().fromJsonable(
						apoolJsonObj
					)
		...
		applyChangesToBase: (c, optAuthor, apoolJsonObj) => {
			...
			aceCallbacksProvider.withCallbacks(
				'applyChangesToBase',
				(callbacks) => {
					if (apoolJsonObj) {
						const wireApool = new AttributePool().fromJsonable(
		...
		prepareUserChangeset: () => {
			...
			if (submittedChangeset) {
				...
				for (const op of Changeset.deserializeOps(cs.ops)) {
					if (op.opcode === '+') {
						const attribs = AttributeMap.fromString(
							op.attribs,
							apool
						)
		...
		applyPreparedChangesetToBase: () => {
			...
			baseAText = Changeset.applyToAText(
				submittedChangeset,
				baseAText,
				apool
			)
