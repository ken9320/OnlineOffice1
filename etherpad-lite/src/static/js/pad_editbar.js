...
exports.padeditbar = new (class {
			...
			if (moduleName === 'none') {
				...
				for (const thisModuleName of this.dropdowns) {
					...
					if (module.hasClass('popup-show')) {
						...
						$(`li[data-key=${thisModuleName}] > a`).addClass(
							'selected'
						)
	...
	setEmbedLinks() {
		...
		if ($('#readonlyinput').is(':checked')) {
			...
			urlParts.pop()
			const readonlyLink = `${urlParts.join('/')}/${
				clientVars.readOnlyId
			}`
			$('#embedinput').val(
	...
	checkAllIconsAreDisplayedInToolbar() {
		...
		if (
			(menuLeft &&
				menuLeft.scrollWidth >
					$('.toolbar').width() - menuRightWidth) ||
			$('.toolbar').width() < 1000
		) {
	...
	_registerDefaultCommands() {
		...
		this.registerAceCommand('clearauthorship', (cmd, ace) => {
			...
			if (rep.selStart[0] === 0 && rep.selStart[1] === 0) {
				...
				if (
					rep.selEnd[0] === lastLineIndex &&
					rep.selEnd[1] === lastChar
				) {
					doPrompt = true
				}
			}
