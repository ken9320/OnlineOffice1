...
exports.getAvailablePlugins = (maxCacheAge) => {
	...
	return new Promise((resolve, reject) => {
		...
		request(
			'https://static.etherpad.org/plugins.json',
			(er, response, plugins) => {
				...
				try {
					...
					logger.error(
						`error parsing plugins.json: ${err.stack || err}`
					)
					plugins = []
				}
...
exports.search = (searchTerm, maxCacheAge) =>
	exports.getAvailablePlugins(maxCacheAge).then((results) => {
		const res = {}
		...
		for (const pluginName in results) {
			...
			if (
				...
				!~results[pluginName].description
					.toLowerCase()
					.indexOf(searchTerm)
			) {
