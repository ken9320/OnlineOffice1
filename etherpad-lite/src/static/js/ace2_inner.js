...
function Ace2Inner(editorInfo, cssManagers) {
	...
	const appendNewSideDivLine = () => {
		...
		lineSpan.appendChild(
			outerDoc.createTextNode(sideDivInner.children.length)
		)
			...
			inCallStackIfNecessary(operationName, () => {
				...
				f({
					...
					applyChangesetToDocument: (
						...
						const oldEventType =
							currentCallStack.editEvent.eventType
	...
	for (const eventType of hooks.callAll(
	...
	const importText = (text, undoable, dontProcess) => {
		...
		if (dontProcess) {
			...
			if (/[\r\t\xa0]/.exec(text)) {
				throw new Error(
					'new raw text must not contain CR, tab, or nbsp'
				)
			}
	...
	const incorporateUserChanges = () => {
		...
		while (j < dirtyRanges.length) {
			...
			if (
				!(
					(a === 0 ||
						getCleanNodeByKey(rep.lines.atIndex(a - 1).key)) &&
					(b === rep.lines.length() ||
		...
		if (selection && !selStart) {
			const selStartFromHook = hooks.callAll(
		...
		if (selection)
			repSelectionChange(
				selStart,
				selEnd,
				selection && selection.focusAtStart
			)
			...
			if (selStart[0] !== selEnd[0]) {
				...
				for (let n = selStart[0] + 1; n < selEnd[0]; n++) {
					hasAttrib =
						...
						rangeHasAttrib(
							[n, 0],
							[n, rep.lines.atIndex(n).text.length]
						)
				}
				...
				hasAttrib =
					hasAttrib &&
					rangeHasAttrib([selEnd[0], 0], [selEnd[0], selEnd[1]])
	...
	const doIncorpLineSplice = (
		...
		if (!(shortOldText.length === 0 && shortNewText.length === 0)) {
			...
			const spliceStartLineStart =
				rep.lines.offsetOfIndex(spliceStartLine)
			...
			const eachAttribRun = (
				...
				for (const op of Changeset.deserializeOps(attribs)) {
					...
					if (
						!(nextIndex <= newTextStart || textIndex >= newTextEnd)
					) {
						func(
			...
			if (justApplyStyles) {
				...
				const spliceEndLineStart =
					rep.lines.offsetOfIndex(spliceEndLine)
				...
				let foundDomAuthor = ''
				eachAttribRun(newAttribs, (start, end, attribs) => {
					const a = AttributeMap.fromString(attribs, rep.apool).get(
						'author'
					)
	...
	const analyzeChange = (
		...
		if (commonStart + commonEnd > oldLen) {
			...
			if (
				hintedCommonEnd >= minCommonEnd &&
				hintedCommonEnd <= maxCommonEnd
			) {
	...
	const updateStyleButtonState = (attribName, hasStyleOnRepSelection) => {
		const $formattingButton = parent.parent
			.$(`[data-key="${attribName}"]`)
			.find('a')
		$formattingButton.toggleClass(
			SELECT_BUTTON_CLASS,
			hasStyleOnRepSelection
		)
	}
	...
	const selectFormattingButtonIfLineHasStyleApplied = (rep) => {
		for (const style of FORMATTING_STYLES) {
			const hasStyleOnRepSelection =
				documentAttributeManager.hasAttributeOnSelectionOrCaretPosition(
					style
				)
	...
	const markNodeClean = (n) => {
		...
		setAssoc(n, 'dirtiness', {
			nodeId: uniqueId(n),
			knownHTML: n.innerHTML
		})
	}
			...
			if (!handled) {
				if (isCaret()) {
					...
					if (caretColumn() <= lineEntry.lineMarker) {
						...
						const prevLineEntry =
							theLine > 0 && rep.lines.atIndex(theLine - 1)
						const prevLineBlank =
							prevLineEntry &&
						...
						if (docChar > 0) {
							if (evt.metaKey || evt.ctrlKey || evt.altKey) {
								...
								while (
									...
									isWordChar(
										rep.alltext.charAt(deleteBackTo - 1)
									)
								) {
									deleteBackTo--
								}
	...
	const handleKeyEvent = (evt) => {
		...
		inCallStackIfNecessary('handleKeyEvent', function () {
			...
			if (!stopped) {
				...
				const padShortcutEnabled =
					parent.parent.clientVars.padShortcutEnabled
				if (
					!specialHandled &&
					isTypeForSpecialKey &&
					altKey &&
				...
				if (
					...
					if (alineAttrs) {
						for (const op of Changeset.deserializeOps(alineAttrs)) {
							const authorId = AttributeMap.fromString(
								op.attribs,
								apool
							).get('author')
				...
				if (
					...
					scheduler.setTimeout(() => {
						...
						const newVisibleLineRange =
							scroll.getVisibleLineRange(rep)
		...
		if (undoModule.enabled) {
			...
			if (whichMethod) {
				...
				undoModule[whichMethod]((backset, selectionInfo) => {
					...
					const oldEvent = currentCallStack.startNewEvent(
						oldEventType,
						true
					)
					return oldEvent
	...
	const getSelection = () => {
		...
		const selection = {
			startPoint: pointFromRangeBound(
