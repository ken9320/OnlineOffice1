	...
	for (const o of exports.deserializeOps(ops)) {
		switch (o.opcode) {
			...
			default:
				assert(
					false,
					`Invalid changeset: Unknown opcode: ${JSON.stringify(
						o.opcode
					)}`
				)
		}
	...
	removeLines(L) {
		...
		if (this._isCurLineInSplice()) {
			if (this._curCol === 0) {
				...
				removed =
					this._curSplice[sline].substring(this._curCol) + removed
	...
	insert(text, L) {
		...
		if (L) {
			...
			if (this._isCurLineInSplice()) {
				...
				this._curSplice[sline] =
					theLine.substring(0, lineCol) + newLines[0]
				this._curLine++
				newLines.splice(0, 1)
...
exports.compose = (cs1, cs2, pool) => {
	...
	assert(
		len2 === unpacked2.oldLen,
		'mismatched composition of two changesets'
	)
		...
		if (csOp.opcode === '=') {
			if (csOp.attribs) {
				...
				const undoBackToAttribs = cachedStrFunc((oldAttribsStr) => {
					const oldAttribs = AttributeMap.fromString(
						oldAttribsStr,
						pool
					)
