...
AttributeManager.prototype = _(AttributeManager.prototype).extend({
  ...
  */
	...
	_setAttributesOnRangeByLine(row, startCol, endCol, attribs) {
		...
		ChangesetUtils.buildKeepToStartOfRange(this.rep, builder, [
			row,
			startCol
		])
  ...
  */
	lineHasMarker(lineNum) {
		return (
			lineAttributes.find(
				(attribute) =>
					this.getAttributeOnLine(lineNum, attribute) !== ''
			) !== undefined
		)
	},
  ...
  */
	getAttributeOnLine(lineNum, attributeName) {
		...
		if (op == null) return ''
		return (
			AttributeMap.fromString(op.attribs, this.rep.apool).get(
				attributeName
			) || ''
		)
	},
