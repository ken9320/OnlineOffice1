...
const makeContentCollector = (
	...
	const _produceLineAttributesMarker = (state) => {
		...
		const attribs = new AttributeMap(apool)
			...
			.update(
				Object.entries(state.lineAttributes).map(([k, v]) => [
					k,
					v || ''
				]),
				true
	...
	cc.collectContent = function (node, state) {
		...
		if (node.nodeType === node.TEXT_NODE) {
			...
			if (tname === 'br') {
				...
				const [startNewLine = true] = hooks.callAll(
				...
				if (
					cls === 'Numbering_20_Symbols' ||
					cls === 'Bullet_20_Symbols'
				) {
					styl = null
					cls = null
				...
				if (collectStyles) {
					...
					if (
						tname === 'u' ||
						(styl &&
							/\btext-decoration:\s*underline\b/i.exec(styl)) ||
						tname === 'ins'
					) {
					...
					if (
						tname === 's' ||
						(styl &&
							/\btext-decoration:\s*line-through\b/i.exec(
								styl
							)) ||
						tname === 'del'
					) {
					...
					if (tname === 'ul' || tname === 'ol') {
						...
						if (!abrowser.chrome)
							oldListTypeOrNull =
								_enterList(state, undefined) || 'none'
					} else if (tname === 'li') {
						state.lineAttributes.start = state.start || 0
						_recalcAttribString(state)
						if (
							state.lineAttributes.list.indexOf('number') !== -1
						) {
							/*
