	...
	;(() => {
		...
		const updateSliderElements = () => {
			...
			$('#ui-slider-handle').css(
				'left',
				(sliderPos * ($('#ui-slider-bar').width() - 2)) /
					(sliderLength * 1.0)
			)
		}
		...
		const setAuthors = (authors) => {
			...
			_.each(authors, (author) => {
				if (author) {
					...
					if (author.name) {
						...
						const textColor =
							colorutils.textColorFromBackgroundColor(
								authorColor,
								clientVars.skinName
							)
						$('<span />')
			...
			if (numAnonymous > 0) {
				const anonymousAuthorString = html10n.get(
					'timeslider.unnamedauthors',
					{ num: numAnonymous }
				)
			...
			if (authors.length === 0) {
				authorsList.append(
					html10n.get('timeslider.toolbar.authorsList')
				)
			}
		}
		...
		fireWhenAllScriptsAreLoaded.push(() => {
			...
			$('#ui-slider-handle').mousedown(function (evt) {
				...
				$(document).mousemove((evt2) => {
					...
					let newloc =
						this.currentLoc + (evt2.clientX - this.startLoc)
				...
				$(document).mouseup((evt2) => {
					...
					$(this).css('left', newloc)
					setSliderPosition(
						Math.floor((newloc * sliderLength) / maxPos)
					)
