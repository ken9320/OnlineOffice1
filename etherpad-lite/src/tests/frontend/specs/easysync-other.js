...
describe('easysync-other', function () {
	...
	describe('make attribs string', function () {
		const testMakeAttribsString = (
			...
			it(`testMakeAttribsString#${testId}`, async function () {
				...
				try {
					expect(
						Changeset.makeAttribsString(opcode, attribs, p)
					).to.equal(correctString)
				} finally {
	...
	describe('other', function () {
		it('testMoveOpsToNewPool', async function () {
			...
			expect(
				Changeset.moveOpsToNewPool('*1+1*0+1', pool1, pool2)
			).to.equal('*0+1*1+1')
		})

		it('testMakeSplice', async function () {
			const t = 'a\nb\nc\n'
			const t2 = Changeset.applyToText(
				Changeset.makeSplice(t, 5, 0, 'def'),
				t
			)
		...
		describe('split/join attribution lines', function () {
			const testSplitJoinAttributionLines = (randomSeed) => {
				...
				it(`testSplitJoinAttributionLines#${randomSeed}`, async function () {
					...
					expect(
						Changeset.splitAttributionLines(theJoined, doc)
					).to.eql(theSplit)
					expect(Changeset.joinAttributionLines(theSplit)).to.equal(
						theJoined
					)
				})
			}
