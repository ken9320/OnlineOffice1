...
describe('Admin > Settings', function () {
	...
	beforeEach(async function () {
		...
		await helper.waitForPromise(
			() => helper.admin$ && helper.admin$('.settings').val().length > 0,
			5000
		)
	})
	...
	it('Are Settings visible, populated, does save work', async function () {
		...
		await helper.waitForPromise(
			() =>
				helper.admin$ &&
				helper.admin$('.settings').val().length === settingsLength + 11,
			20000
		)
		...
		helper
			.admin$('.settings')
			.val((_, text) => text.replace('/* test */\n', ''))
		await helper.waitForPromise(
			() => settingsLength === helper.admin$('.settings').val().length
		)
		await save()
		...
		await helper.waitForPromise(
			() =>
				helper.admin$ &&
				helper.admin$('.settings').val().length === settingsLength &&
				settings === helper.admin$('.settings').val(),
			20000
		)
	})
