	...
	describe('follow & compose', function () {
		const testFollow = (randomSeed) => {
			it(`testFollow#${randomSeed}`, async function () {
				...
				const afb = Changeset.checkRep(
					Changeset.follow(cs1, cs2, false, p)
				)
				const bfa = Changeset.checkRep(
					Changeset.follow(cs2, cs1, true, p)
				)
	...
	describe('followAttributes & composeAttributes', function () {
		...
		const testFollow = (a, b, afb, bfa, merge) => {
			it(`manual #${++n}`, async function () {
				...
				expect(Changeset.composeAttributes(a, afb, true, p)).to.equal(
					merge
				)
				expect(Changeset.composeAttributes(b, bfa, true, p)).to.equal(
					merge
				)
			})
		}
