...
describe('indentation button', function () {
	...
	it('indent text with keypress', async function () {
		...
		await helper.waitForPromise(
			() => inner$('div').first().find('ul li').length === 1
		)
	})
	...
	it('keeps the indent on enter for the new line', async function () {
		...
		await helper.waitFor(
			() => inner$('div span').first().text().indexOf('line 2') === -1
		)
	...
	it(
		...
		async function () {
			...
			// curly braces
			const $lineWithCurlyBraces = inner$('div')
				.first()
				.next()
				.next()
				.next()
			...
			pressEnter()
			const $lineAfterCurlyBraces = inner$('div')
				.first()
				.next()
				.next()
				.next()
			...
			pressEnter()
			const $lineAfterParenthesis = inner$('div')
				.first()
				.next()
				.next()
				.next()
	...
	it(
		...
		async function () {
			...
			// type a bit, make a line break and type again
			const $firstTextElement = inner$('div').first()
			$firstTextElement.sendkeys(
				"  line with some indentation and ':'{enter}"
			)
	...
	it(
		...
		async function () {
			...
			await helper.waitForPromise(
				() => inner$('div').first().text().trim() === 'First'
			)
			...
			await helper.waitForPromise(
				() => inner$('div').first().find('b').length === 1
			)
			...
			$outdentButton.click()
			await helper.waitForPromise(
				() => inner$('div').first().find('ul li').length === 0
			)
	...
	xit('makes text indented and outdented', async function () {
		...
		const $outdentButton = helper
			.$getPadChrome()
			.find('.buttonicon-outdent')
		$outdentButton.click()
		$outdentButton.click()
    ...
    */
		...
		const secondTextElement = $('iframe')
			.contents()
			.find('iframe')
			.contents()
			.find('iframe')
