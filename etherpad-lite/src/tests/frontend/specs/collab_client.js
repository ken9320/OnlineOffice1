...
describe('Messages in the COLLABROOM', function () {
	...
	it('bug #4978 regression test', async function () {
		...
		const finishSend = (() => {
			...
			let startSend
			sendStarted = new Promise((resolve) => {
				startSend = resolve
			})
			let finishSend
			const sendP = new Promise((resolve) => {
				finishSend = resolve
			})
		...
		await helper.multipleUsers.performAsOtherUser(
			async () => await replaceLineText(1, user2Text)
		)
		...
		await helper.waitForPromise(
			() => helper.padInner$('div').eq(1).text() === user2Text
		)

		// Users 1 and 2 make some more changes.
		await helper.multipleUsers.performAsOtherUser(
			async () => await replaceLineText(3, user2Text)
		)
		...
		const assertContent = async () =>
			await helper.waitForPromise(() => {
				...
				return expectedLines.every(
					(txt, i) => helper.padInner$('div').eq(i).text() === txt
				)
			})
		await assertContent()
