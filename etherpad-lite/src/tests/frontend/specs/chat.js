...
describe('Chat messages and UI', function () {
	...
	it(
		'opens chat, sends a message, makes sure it exists ' +
			'on the page and hides chat',
		async function () {
			this.timeout(3000)
			...
			expect(helper.chatTextParagraphs().text()).to.be(
				`${username}${time} ${chatValue}\n`
			)
	...
	it(
		'makes chat stick to right side of the screen via settings, ' +
			'remove sticky via settings, close it',
		async function () {
			this.timeout(5000)
	...
	it(
		'makes chat stick to right side of the screen via icon on the top' +
			' right, remove sticky via icon, close it',
		async function () {
			this.timeout(5000)
			await helper.showChat()
