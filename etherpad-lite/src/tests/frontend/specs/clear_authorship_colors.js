...
describe('clear authorship colors button', function () {
	...
	it('makes text clear authorship colors', async function () {
		...
		await helper.waitForPromise(
			() =>
				inner$('div span').first().attr('class').indexOf('author') !==
				-1
		)
		...
		const $clearauthorshipcolorsButton = chrome$(
		...
		const hasAuthorClass =
			inner$('div').first().attr('class').indexOf('author') !== -1
		expect(hasAuthorClass).to.be(false)

		await helper.waitForPromise(
			() =>
				chrome$('div.disconnected').attr('class').indexOf('visible') ===
				-1
		)
	})
	...
	it('clears authorship when first line has line attributes', async function () {
		...
		await helper.waitForPromise(
			() => helper.padInner$('[class*="author-"]').length > 0
		)
