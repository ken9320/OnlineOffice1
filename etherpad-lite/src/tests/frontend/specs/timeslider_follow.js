...
describe('timeslider follow', function () {
	...
	it("content as it's added to timeslider", async function () {
		...
		await helper.waitForPromise(
			() => helper.contentWindow().location.hash === '#0'
		)
	...
	it('only to lines that exist in the pad view, regression test for #4389', async function () {
		...
		const oldYPosition = helper
			.contentWindow()
			.$('#editorcontainerbox')[0].scrollTop
...
const hasFollowedToLine = (lineNum) => {
	const scrollPosition = helper
		.contentWindow()
		.$('#editorcontainerbox')[0].scrollTop
	const lineOffset = helper
		.contentWindow()
		.$('#innerdocbody')
		.find(`div:nth-child(${lineNum})`)[0].offsetTop
