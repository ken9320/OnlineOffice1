...
describe('Automatic pad reload on Force Reconnect message', function () {
	...
	context('and user clicks on Cancel', function () {
		beforeEach(async function () {
			const $errorMessageModal = helper.padChrome$(
			...
			await helper.waitForPromise(
				() =>
					helper
						.padChrome$('#connectivity .userdup')
						.is(':visible') === true
			)
		})
	...
	context(
		'and user does not click on Cancel until timer expires',
		function () {
			it('reloads the pad', async function () {
				this.timeout(10000)
				await new Promise((resolve) =>
					$originalPadFrame.one('load', resolve)
				)
			})
		}
	)
