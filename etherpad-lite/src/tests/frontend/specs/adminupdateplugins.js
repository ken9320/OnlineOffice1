...
describe('Plugins page', function () {
	...
	it('Lists some plugins', async function () {
		await helper.waitForPromise(
			() => helper.admin$('.results').children().length > 50,
			20000
		)
	})
	...
	it('Searches for plugin', async function () {
		...
		await helper.waitForPromise(
			() => helper.admin$('.results').children().length > 0,
			10000
		)
		await helper.waitForPromise(
			() => helper.admin$('.results').children().length < 300,
			10000
		)
	})
	...
	it('Attempt to Update a plugin', async function () {
		...
		await helper.waitForPromise(
			() =>
				helper.admin$('.ep_align .version').text().split('.').length >=
				2
		)

		const minorVersionBefore = parseInt(
			helper.admin$('.ep_align .version').text().split('.')[1]
		)

		if (!minorVersionBefore) {
			throw new Error(
				'Unable to get minor number of plugin, is the plugin installed?'
			)
		}
	...
	it('Attempt to Install a plugin', async function () {
		...
		if (helper.admin$('.ep_headings2 .do-install').is(':visible') === false)
			this.skip()

		helper.admin$('.ep_headings2 .do-install').click()
		// ensure install has attempted to be started
		await helper.waitForPromise(
			() => helper.admin$('.ep_headings2 .do-install').length !== 0,
			120000
		)
		// ensure its not showing installing any more
		await helper.waitForPromise(
			() => helper.admin$('.ep_headings2 .message').text() === '',
			180000
		)
		// ensure uninstall button is visible
		await helper.waitForPromise(
			() => helper.admin$('.ep_headings2 .do-uninstall').length !== 0,
			120000
		)
	})
	...
	it('Attempt to Uninstall a plugin', async function () {
		...
		// ensure its gone
		await helper.waitForPromise(
			() => helper.admin$('.ep_headings2').length === 0,
			240000
		)
