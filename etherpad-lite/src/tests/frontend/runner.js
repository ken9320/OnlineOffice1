	...
	(async () => {
		...
		const customRunner = (runner) => {
			...
			const autoscroll = (newElement) => {
				...
				const scroll =
					...
					(() => {
						const offsetTopAbs =
							newElement.getBoundingClientRect().top
			...
			runner.on('pass', (test) => {
				...
				stats.passes++
				append(
					`-> [green]PASSED[clear] : ${test.title}   ${test.duration} ms`
				)
			})
		...
		window.testRunnerRequire = {
			define(...args) {
				...
				if (args.length !== 1)
					throw new Error(
						'unexpected args passed to testRunnerRequire.define'
					)
				const [origDefs] = args
				const defs = {}
		...
		const appendToLog = (msg) => {
			...
			const scrolledToBottom =
				$log[0].scrollHeight <=
				$log[0].scrollTop + $log[0].clientHeight + 5
			const $msg = $('<div>')
		...
		const makeDesc = (spec) =>
			`${spec
				.replace(
					/^ep_etherpad-lite\/tests\/frontend\/specs\//,
					'<core> '
				)
				.replace(
				...
				try {
					await new Promise((resolve, reject) =>
						require(spec, (module) => {
							if (module == null)
								return reject(
									new Error(`failed to load module ${spec}`)
								)
							resolve()
						})
					)
		...
		for (const spec of specs) {
			...
			try {
				describe(desc, function () {
					for (const [fn, args] of mochaCalls.get(spec))
						window[fn](...args)
				})
			} catch (err) {
				$msg.append($('<b>').css('color', 'red').text(' FAILED'))
