...
const testCases = [
	...
	{
		description:
			'First item being an UL then subsequent being OL will fail',
	...
	{
		description:
			"Don't collapse spaces that preceed/follow a empty paragraph",
	...
	{
		description:
			"Don't collapse non-breaking spaces that follow a paragraph",
		html: '<html><body>something<p></p>&nbsp;&nbsp;&nbsp;something<br></body></html>',
		wantAlines: ['+9', '', '+c'],
		wantText: ['something', '', '   something']
	},
			...
			before(async function () {
				...
				for (const aline of tc.wantAlines) {
					for (const op of Changeset.deserializeOps(aline)) {
						for (const n of attributes.decodeAttribString(
							op.attribs
						)) {
				...
				const cc = contentcollector.makeContentCollector(
					true,
					null,
					apool
				)
			...
			it('attributes are sorted in canonical order', async function () {
				...
				for (const aline of result.lineAttribs) {
					...
					for (const op of Changeset.deserializeOps(aline)) {
						...
						wantAlineAttribs.push(
							attributes.sort([...gotOpAttribs])
						)
					}
				}
