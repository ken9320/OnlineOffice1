...
const endPoint = (point, version) =>
	`/api/${version || apiVersion}/${point}?apikey=${apiKey}`
...
describe(__filename, function () {
	...
	describe('Sanity checks', function () {
		it('errors with invalid APIKey', async function () {
			...
			await agent
				.get(`/api/${apiVersion}/createPad?apikey=password&padID=test`)
				.expect(401)
		})
	})
  ...
  */
	...
	describe('Tests', function () {
		...
		it('creates a new Pad with text', async function () {
			const res = await agent
				.get(
					`${endPoint('createPad')}&padID=${testPadId}&text=testText`
				)
				.expect(200)
				.expect('Content-Type', /json/)
			assert.equal(res.body.code, 0)
		...
		it('Pad with white space between list items', async function () {
			let res = await agent
				.get(
					`${endPoint(
						'setHTML'
					)}&padID=${testPadId}&html=${ulSpaceHtml}`
				)
				.expect(200)
				.expect('Content-Type', /json/)
			assert.equal(res.body.code, 0)
...
const createNewPadWithHtml = async (padId, html) => {
	...
	await agent.post(endPoint('setHTML')).send({
		padID: padId,
		html
	})
}
