		...
		if (line.listLevel) {
			...
			if (
				...
				const exists = _.find(
					openLists,
					(item) =>
						item.level === line.listLevel &&
						item.type === line.listTypeName
				)
				if (!exists) {
					let prevLevel = 0
			...
			if (
				...
				for (let diff = nextLevel; diff < line.listLevel; diff++) {
					openLists = openLists.filter(
						(el) =>
							el.level !== diff && el.type !== line.listTypeName
					)
