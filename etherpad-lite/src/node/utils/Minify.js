		...
		mockResponse = {
			writeHead: (_status, _headers) => {
				...
				for (const header in _headers) {
					if (
						Object.prototype.hasOwnProperty.call(_headers, header)
					) {
	...
	Promise.all(
		locations.map(async (loc) => {
			try {
				...
				logger.debug(
					`requestURI(${JSON.stringify(loc)}, ${JSON.stringify(
						method
					)}, ` +
						`${JSON.stringify(headers)}) failed: ${
							err.stack || err
						}`
				)
				return [500, headers, '']
...
const minify = async (req, res) => {
	...
	const newLocation =
		compatPaths[
			filename.replace(/^plugins\/ep_etherpad-lite\/static\//, '')
		]
	if (newLocation != null) {
		logger.warn(
			...
			await Promise.all(
				files.map(async (filename) => {
					...
					if (
						...
						stats.mtime > latestModification
					) {
