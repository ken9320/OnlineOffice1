...
stats.gauge('totalUsers', () =>
	socketio ? Object.keys(socketio.sockets.sockets).length : 0
)
	...
	async enqueue(ch, task) {
		...
		const pc = p
			...
			.then(() => {
				...
				if (this._promiseChains.get(ch) === pc)
					this._promiseChains.delete(ch)
			})
...
exports.handleDisconnect = async (socket) => {
	...
	accessLogger.info(
		'[LEAVE]' +
			...
			` socket:${socket.id}` +
			` IP:${
				settings.disableIPlogging ? 'ANONYMOUS' : socket.request.ip
			}` +
...
exports.handleMessage = async (socket, message) => {
	...
	if (
		(await hooks.aCallAll('handleMessage', context)).some((m) => m == null)
	) {
		return
	}
				...
				try {
					switch (type) {
						case 'USER_CHANGES':
							...
							await padChannels.enqueue(thisSession.padId, {
								socket,
								message
							})
							break
...
const handleGetChatMessages = async (socket, { data: { start, end } }) => {
	...
	if (!Number.isInteger(end))
		throw new Error(`missing or invalid end: ${end}`)
...
const handleUserChanges = async (socket, message) => {
	...
	try {
		...
		for (const op of Changeset.deserializeOps(
			...
			const opAuthorId = AttributeMap.fromString(
				op.attribs,
				wireApool
			).get('author')
		...
		while (r < pad.getHeadRevisionNumber()) {
			...
			rebasedChangeset = Changeset.follow(
				c,
				rebasedChangeset,
				false,
				pad.pool
		...
		if (Changeset.oldLen(rebasedChangeset) !== prevText.length) {
			throw new Error(
				`Can't apply changeset ${rebasedChangeset} with oldLen ` +
					`${Changeset.oldLen(
						rebasedChangeset
					)} to document of length ${prevText.length}`
			)
		}
		...
		roomSockets.map(async (socket) => {
			...
			while (sessioninfo.rev < pad.getHeadRevisionNumber()) {
				...
				try {
					...
					messageLogger.error(
						`Failed to notify user of new revision: ${
							err.stack || err
						}`
					)
					return
...
const _correctMarkersInPad = (atext, apool) => {
	...
	for (const op of Changeset.deserializeOps(atext.attribs)) {
		...
		const hasMarker = AttributeManager.lineAttributes.some((a) =>
			attribs.has(a)
		)
		if (hasMarker) {
...
const handleClientReady = async (socket, message) => {
	...
	if (message.reconnect) {
		...
		try {
			...
			const attribsForWire = Changeset.prepareForWire(
				atext.attribs,
				pad.pool
			)
...
const handleChangesetRequest = async (
	socket,
	{ data: { granularity, start, requestID } }
) => {
...
const getChangesetInfo = async (pad, startNum, endNum, granularity) => {
	...
	const [lines] = await Promise.all([
		...
		...compositesChangesetNeeded.map(async (item) => {
			const changeset = await composePadChangesets(
				pad,
				item.start,
				item.end
			)
	...
	for (
		...
		const forwards2 = Changeset.moveOpsToNewPool(
			forwards,
			pad.apool(),
			apool
		)
