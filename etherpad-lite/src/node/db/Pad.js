...
class Pad {
	...
	async appendRevision(aChangeset, authorId = '') {
		const newAText = Changeset.applyToAText(
			aChangeset,
			this.atext,
			this.pool
		)
	...
	async getAllAuthorColors() {
		...
		await Promise.all(
			authorIds.map((authorId) =>
				authorManager.getAuthorColorId(authorId).then((colorId) => {
	...
	async spliceText(start, ndel, ins, authorId = '') {
		if (start < 0)
			throw new RangeError(
				`start index must be non-negative (is ${start})`
			)
		if (ndel < 0)
			throw new RangeError(
	...
	async getChatMessage(entryNum) {
		...
		message.displayName = await authorManager.getAuthorName(
			message.authorId
		)
		return message
	}
	...
	async getChatMessages(start, end) {
		...
		return entries.filter((entry) => {
			...
			if (!pass) {
				console.warn(
					`WARNING: Found broken chat entry in pad ${this.id}`
				)
			}
			return pass
		})
	...
	async copy(destinationID, force) {
		...
		const destGroupID = await this.checkIfGroupExistAndReturnIt(
			destinationID
		)
	...
	async remove() {
		...
		p.push(
			readOnlyManager.getReadOnlyId(padID).then(async (readonlyID) => {
		...
		p.push(
			promises.timesLimit(this.chatHead + 1, 500, async (i) => {
		...
		p.push(
			promises.timesLimit(this.head + 1, 500, async (i) => {
	...
	async addSavedRevision(revNum, savedById, label) {
		...
		for (const i in this.savedRevisions) {
			if (
				this.savedRevisions[i] &&
				this.savedRevisions[i].revNum === revNum
			) {
				return
			}
		}
