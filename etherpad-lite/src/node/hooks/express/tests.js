	...
	await Promise.all(
		dirents.map(async (dirent) => {
			if (dirent.isDirectory()) {
				const subdirSpecs = await findSpecs(
					path.join(specDir, dirent.name)
				)
				specs.push(
					...subdirSpecs.map((spec) => `${dirent.name}/${spec}`)
				)
				return
			}
...
exports.expressPreSession = async (hookName, { app }) => {
	app.get('/tests/frontend/frontendTestSpecs.json', (req, res, next) => {
		;(async () => {
				...
				Object.entries(plugins.plugins).map(async ([plugin, def]) => {
					...
					for (const spec of await findSpecs(
						...
						if (
							...
							continue
						modules.push(
							`${plugin}/${specDir}/${spec.replace(/\.js$/, '')}`
						)
					}
				})
			)
			...
			console.debug(
				'Sent browser the following test spec modules:',
				modules
			)
			res.json(modules)
