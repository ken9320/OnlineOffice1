...
const resources = {
	...
	group: {
		...
		listPads: {
			...
			responseSchema: {
				padIDs: { type: 'array', items: { type: 'string' } }
			}
		},
		createPad: {
	...
	session: {
		...
		info: {
			...
			responseSchema: {
				info: { $ref: '#/components/schemas/SessionInfo' }
			}
		}
	},
	...
	pad: {
		...
		getChatHead: {
			...
			responseSchema: {
				chatHead: { $ref: '#/components/schemas/Message' }
			}
		},
		appendChatMessage: {
			...
			for (const funcName of Object.keys(apiHandler.version[version])) {
				const handler = async (c, req, res) => {
					...
					if (logger.isDebugEnabled()) {
						logger.debug(
							`REQUEST, v${version}:${funcName}, ${JSON.stringify(
								fields
							)}`
						)
					}
					...
					if (logger.isDebugEnabled()) {
						logger.debug(
							`RESPONSE, ${funcName}, ${JSON.stringify(response)}`
						)
					}
...
const getApiRootForVersion = (version, style = APIPathStyle.FLAT) =>
	`/${style}/${version}`
