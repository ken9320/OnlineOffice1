...
const aCallFirst = (hookName, context, pred = null) =>
	new Promise((resolve, reject) => {
		hooks.aCallFirst(
			hookName,
			context,
			(err, r) => (err != null ? reject(err) : resolve(r)),
			pred
		)
	})

const aCallFirst0 = async (hookName, context, pred = null) =>
...
exports.normalizeAuthzLevel = (level) => {
	...
	switch (level) {
		...
		default:
			httpLogger.warn(
				`Unknown authorization level '${level}', denying access`
			)
	}
	return false
}
...
const checkAccess = async (req, res, next) => {
	...
	try {
		results = await aCallFirst(
			...
			(r) =>
				skip ||
				(r != null && r.filter((x) => !requireAdmin || !x).length > 0)
		)
	} catch (err) {
		httpLogger.error(
	...
	const authorize = async () => {
		...
		if (requireAdmin && !req.session.user.is_admin)
			return await grant(false)
		if (!settings.requireAuthorization) return await grant('create')
		return await grant(
			await aCallFirst0('authorize', {
				req,
				res,
				next,
				resource: req.path
