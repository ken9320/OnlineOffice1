...
exports.expressCreateServer = (hookName, args, cb) => {
	args.app.get('/admin/plugins', (req, res) => {
		res.send(
			eejs.require('ep_etherpad-lite/templates/admin/plugins.html', {
	...
	args.app.get('/admin/plugins/info', (req, res) => {
		...
		res.send(
			eejs.require('ep_etherpad-lite/templates/admin/plugins-info.html', {
				gitCommit,
				epVersion,
				installedPlugins: `<pre>${plugins
					.formatPlugins()
					.replace(/, /g, '\n')}</pre>`,
				installedParts: `<pre>${plugins.formatParts()}</pre>`,
				installedServerHooks: `<div>${plugins.formatHooks(
					'hooks',
					true
				)}</div>`,
				installedClientHooks: `<div>${plugins.formatHooks(
					'client_hooks',
					true
				)}</div>`,
...
exports.socketio = (hookName, args, cb) => {
	...
	io.on('connection', (socket) => {
		const { session: { user: { is_admin: isAdmin } = {} } = {} } =
			socket.conn.request
		if (!isAdmin) return
		...
		socket.on('checkUpdates', async () => {
			...
			try {
				const results = await installer.getAvailablePlugins(
					/* maxCacheAge:*/ 60 * 10
				)

				const updatable = Object.keys(pluginDefs.plugins).filter(
					(plugin) => {
						if (!results[plugin]) return false

						const latestVersion = results[plugin].version
						const currentVersion =
							pluginDefs.plugins[plugin].package.version
		...
		socket.on('search', async (query) => {
			try {
				const results = await installer.search(
					query.searchTerm,
					/* maxCacheAge:*/ 60 * 10
				)
